"use client";

import Link from "next/link";
import { useEffect, useRef, useState } from "react";

type RevealProps = {
  children: React.ReactNode;
  className?: string;
  delay?: number;
};

function Reveal({ children, className = "", delay = 0 }: RevealProps) {
  const ref = useRef<HTMLDivElement | null>(null);
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const el = ref.current;
    if (!el) return;

    const root = document.querySelector(".snap-container");

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setVisible(true);
          observer.unobserve(entry.target);
        }
      },
      {
        root,
        threshold: 0.18,
      }
    );

    observer.observe(el);

    return () => observer.disconnect();
  }, []);

  return (
    <div
      ref={ref}
      className={className}
      style={{
        opacity: visible ? 1 : 0,
        transform: visible ? "translateY(0px)" : "translateY(40px)",
        transition: `opacity 0.8s cubic-bezier(0.22,1,0.36,1) ${delay}ms, transform 0.8s cubic-bezier(0.22,1,0.36,1) ${delay}ms`,
      }}
    >
      {children}
    </div>
  );
}

export default function Home() {
  const [showHeader, setShowHeader] = useState(false);
  const [scrollProgress, setScrollProgress] = useState(0);
  const scrollRef = useRef<HTMLElement | null>(null);

  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;

    const handleScroll = () => {
      const scrollTop = container.scrollTop;
      const scrollHeight = container.scrollHeight - container.clientHeight;

      setShowHeader(scrollTop > 200);

      if (scrollHeight > 0) {
        setScrollProgress((scrollTop / scrollHeight) * 100);
      }
    };

    handleScroll();
    container.addEventListener("scroll", handleScroll);

    return () => container.removeEventListener("scroll", handleScroll);
  }, []);

  return (
    <main
      ref={scrollRef}
      className="snap-container overflow-x-hidden bg-black text-white"
    >
      <header
        className={`fixed top-0 left-0 z-50 w-full transition-all duration-500 ${
          showHeader
            ? "translate-y-0 opacity-100"
            : "-translate-y-full pointer-events-none opacity-0"
        }`}
      >
        <div
          className="absolute top-0 left-0 h-[2px] bg-white"
          style={{ width: `${scrollProgress}%` }}
        />

        <div className="border-b border-white/10 bg-black/95 backdrop-blur-xl">
          <div className="mx-auto hidden max-w-7xl items-center justify-center px-6 py-5 md:flex">
            <nav className="flex gap-8 text-[15px] text-white/80 lg:gap-12">
              <a href="#hero" className="hover:text-white">
                Ana Sayfa
              </a>
              <a href="#about" className="hover:text-white">
                Hakkımızda
              </a>
              <a href="#services" className="hover:text-white">
                Hizmetler
              </a>
              <a href="#projects" className="hover:text-white">
                Projeler
              </a>
              <a href="#contact" className="hover:text-white">
                İletişim
              </a>
            </nav>
          </div>
        </div>
      </header>

      <section
        id="hero"
        className="snap-section relative min-h-[100dvh] overflow-hidden"
      >
        <img
          src="/hero-bg.png"
          className="absolute inset-0 h-full w-full object-cover"
          alt=""
        />

        <div className="absolute inset-0 bg-black/20" />

        <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-[1400px] flex-col px-5 pt-5 sm:px-6 md:px-8 lg:px-6 lg:pt-6">
          <div className="flex items-center justify-between lg:hidden">
            <div className="w-[132px] sm:w-[150px]">
              <img
                src="/kucuk-logo.png"
                alt="GRGN Creative"
                className="w-full translate-y-[4px] object-contain"
              />
            </div>

            <a
              href="#contact"
              className="rounded-full border border-white/15 bg-white/[0.05] px-4 py-2 text-xs font-medium text-white/90 backdrop-blur-xl transition hover:bg-white/[0.08]"
            >
              İletişim
            </a>
          </div>

          <div className="hidden justify-center lg:flex">
            <div className="flex h-[58px] w-[900px] items-center rounded-full border border-white/20 bg-white/[0.05] px-10 shadow-[0_0_35px_rgba(255,255,255,0.08)] backdrop-blur-xl">
              <div className="flex w-full items-center justify-between">
                <div className="w-[190px] shrink-0">
                  <img
                    src="/kucuk-logo.png"
                    alt="GRGN Creative"
                    className="w-full translate-y-[8px] object-contain"
                  />
                </div>

                <nav className="flex items-center gap-12 text-[15px] font-medium text-white/85">
                  <a href="#hero" className="transition hover:text-white">
                    Ana Sayfa
                  </a>

                  <a href="#about" className="transition hover:text-white">
                    Hakkımızda
                  </a>

                  <a href="#services" className="transition hover:text-white">
                    Hizmetler
                  </a>

                  <a href="#projects" className="transition hover:text-white">
                    Projeler
                  </a>

                  <a href="#contact" className="transition hover:text-white">
                    İletişim
                  </a>
                </nav>
              </div>
            </div>
          </div>

          <div className="flex flex-1 items-center">
            <div className="max-w-[760px] pt-10 sm:pt-14 md:pt-16">
              <Reveal>
                <h1 className="text-[52px] font-semibold leading-[0.92] tracking-[-0.05em] sm:text-[68px] md:text-[82px] lg:text-[90px]">
                  Premium
                </h1>
              </Reveal>

              <Reveal delay={100}>
                <h2 className="text-[48px] font-light leading-[0.92] tracking-[-0.05em] text-white/90 sm:text-[64px] md:text-[78px] lg:text-[88px]">
                  Yaratıcılık <span className="font-semibold">Ajansı</span>
                </h2>
              </Reveal>

              <Reveal delay={180}>
                <p className="mt-6 max-w-[600px] text-base leading-7 text-white/70 sm:text-lg sm:leading-8">
                  Markanızı zirveye taşımak için tasarım, dijital ve marka
                  stratejisi alanında uzman bir ajansız.
                </p>
              </Reveal>

              <Reveal delay={260}>
                <div className="mt-8 sm:mt-10">
                  <a
                    href="#services"
                    className="rounded-full border border-blue-400 px-7 py-3 text-sm text-white transition hover:bg-blue-500/20 sm:px-8 sm:text-base"
                  >
                    Hizmetler
                  </a>
                </div>
              </Reveal>
            </div>
          </div>
        </div>
      </section>

      <section
        id="services"
        className="snap-section relative flex min-h-[100dvh] items-center overflow-hidden px-5 py-20 sm:px-6 md:px-10 md:py-28"
      >
        <div className="absolute inset-0 bg-[linear-gradient(180deg,#02040b_0%,#030611_45%,#010208_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(55,90,210,0.10),transparent_25%),radial-gradient(circle_at_80%_30%,rgba(255,255,255,0.05),transparent_20%),radial-gradient(circle_at_70%_80%,rgba(45,80,200,0.08),transparent_28%)]" />
        <div className="absolute inset-0 opacity-[0.04] [background-image:radial-gradient(rgba(255,255,255,0.8)_0.45px,transparent_0.45px)] [background-size:4px_4px]" />

        <div className="relative z-10 mx-auto w-full max-w-7xl">
          <Reveal>
            <h2 className="mb-10 text-center text-4xl font-semibold tracking-tight sm:text-5xl">
              Hizmetler
            </h2>
          </Reveal>

          <div className="grid gap-8 md:grid-cols-2">
            <Reveal delay={0}>
              <ServiceCard
                img="/hizmet-marka.png"
                title="Kurumsal Kimlik"
                desc="Logo, renk sistemi ve görsel dil tasarımı."
                href="/hizmetler/kurumsal-kimlik"
              />
            </Reveal>

            <Reveal delay={120}>
              <ServiceCard
                img="/hizmet-sosyal-medya.png"
                title="Sosyal Medya Tasarımı"
                desc="Marka kimliğine uygun içerik tasarımları."
                href="/hizmetler/sosyal-medya-tasarimi"
              />
            </Reveal>

            <Reveal delay={220}>
              <ServiceCard
                img="/hizmet-ambalaj.png"
                title="Ambalaj Tasarımı"
                desc="Ürün ve paketleme tasarımı."
                href="/hizmetler/ambalaj-tasarimi"
              />
            </Reveal>

            <Reveal delay={320}>
              <ServiceCard
                img="/hizmet-web.png"
                title="Web Tasarım"
                desc="Modern arayüz ve kullanıcı deneyimi."
                href="/hizmetler/web-tasarim"
              />
            </Reveal>
          </div>
        </div>
      </section>

      <section
        id="projects"
        className="snap-section relative flex min-h-[100dvh] items-center overflow-hidden px-5 py-20 sm:px-6 md:px-10 md:py-28"
      >
        <div className="absolute inset-0 bg-[linear-gradient(180deg,#02040b_0%,#030611_45%,#010208_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(55,90,210,0.10),transparent_24%),radial-gradient(circle_at_82%_30%,rgba(255,255,255,0.05),transparent_18%),radial-gradient(circle_at_72%_80%,rgba(45,80,200,0.08),transparent_28%)]" />
        <div className="absolute inset-0 opacity-[0.04] [background-image:radial-gradient(rgba(255,255,255,0.8)_0.45px,transparent_0.45px)] [background-size:4px_4px]" />

        <div className="absolute left-[-8%] top-[10%] h-[340px] w-[340px] rounded-full bg-blue-500/10 blur-3xl" />
        <div className="absolute right-[-10%] bottom-[8%] h-[420px] w-[420px] rounded-full bg-white/5 blur-3xl" />

        <div className="relative z-10 mx-auto w-full max-w-7xl">
          <Reveal>
            <div className="mb-10 flex flex-col gap-6 md:mb-12 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="mb-4 text-sm font-medium uppercase tracking-[0.22em] text-white/45">
                  Seçili Projeler
                </p>
                <h2 className="text-4xl font-semibold tracking-tight text-white sm:text-5xl md:text-6xl">
                  Projeler
                </h2>
                <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-400 sm:text-lg sm:leading-8 md:mt-6 md:text-[22px] md:leading-9">
                  Markalar için ürettiğimiz yaratıcı çözümleri ve dijital
                  deneyimleri öne çıkan projeler üzerinden keşfedin.
                </p>
              </div>

              <Link
                href="/projeler"
                className="hidden rounded-full border border-white/12 bg-white/[0.04] px-6 py-3 text-sm text-white/90 transition hover:border-white/20 hover:bg-white/[0.08] md:inline-flex"
              >
                Tüm Projeleri Gör
              </Link>
            </div>
          </Reveal>

          <div className="grid items-stretch gap-8 md:grid-cols-2 xl:grid-cols-3">
            <Reveal delay={0} className="h-full">
              <ProjectCard
                img="/mericbey-project.png"
                title="Meriçbey Jersey Süt Çiftliği"
                category="Marka Stratejisi / İçerik Sistemi"
                desc="Premium konumlandırma, güven odaklı iletişim ve Instagram üzerinden ilerleyen satış sistemini destekleyen marka kurgusu."
                href="/projeler/mericbey-jersey-sut-ciftligi"
              />
            </Reveal>

            <Reveal delay={120} className="h-full">
              <ProjectCard
                img="/torkoop-project.png"
                title="Torkoop"
                category="Kurumsal Kimlik / Etiket Tasarımı"
                desc="Torbalı merkezli üretim yapan kooperatif için kurumsal logo ve ürün etiketleri tasarlanarak markanın rafta daha güçlü görünmesi sağlandı."
                href="/projeler/torkoop"
              />
            </Reveal>

            <Reveal delay={220} className="h-full">
              <ProjectCard
                img="/ortbas-project.png"
                title="Örtbas"
                category="Yaratıcı Yönetmenlik / Kısa Film"
                desc="“Gölgelerden kaçamazsın.” Yönetmenlik, kurgu ve hikaye geliştirme süreçlerinin tamamı tarafımdan yürütülen kısa film projesi."
                href="/projeler/ortbas"
              />
            </Reveal>
          </div>

          <Reveal delay={300}>
            <div className="mt-10 md:hidden">
              <Link
                href="/projeler"
                className="inline-flex rounded-full border border-white/12 bg-white/[0.04] px-6 py-3 text-sm text-white/90 transition hover:border-white/20 hover:bg-white/[0.08]"
              >
                Tüm Projeleri Gör
              </Link>
            </div>
          </Reveal>
        </div>
      </section>

      <section
        id="about"
        className="snap-section relative flex min-h-[100dvh] items-center overflow-hidden px-5 py-20 sm:px-6 md:px-10 md:py-28"
      >
        <div className="absolute inset-0 bg-[linear-gradient(180deg,#02040b_0%,#030611_45%,#010208_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(55,90,210,0.10),transparent_24%),radial-gradient(circle_at_82%_28%,rgba(255,255,255,0.05),transparent_18%),radial-gradient(circle_at_70%_78%,rgba(45,80,200,0.08),transparent_28%)]" />
        <div className="absolute inset-0 opacity-[0.04] [background-image:radial-gradient(rgba(255,255,255,0.8)_0.45px,transparent_0.45px)] [background-size:4px_4px]" />

        <div className="absolute -left-[10%] top-[10%] h-[340px] w-[340px] rounded-full bg-blue-500/10 blur-3xl" />
        <div className="absolute right-[-8%] bottom-[5%] h-[420px] w-[420px] rounded-full bg-white/5 blur-3xl" />

        <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 md:grid-cols-[1fr_1.1fr] md:gap-20">
          <Reveal>
            <div>
              <p className="mb-4 text-sm font-medium uppercase tracking-[0.22em] text-white/45">
                GRGN Creative
              </p>

              <h2 className="text-4xl font-semibold tracking-tight text-white sm:text-5xl md:text-6xl">
                Hakkımızda
              </h2>

              <div className="mt-6 h-px w-24 bg-white/15" />
            </div>
          </Reveal>

          <Reveal delay={140}>
            <div className="rounded-[30px] border border-white/10 bg-white/[0.03] p-7 shadow-[0_0_40px_rgba(255,255,255,0.03)] backdrop-blur-sm sm:p-8 md:p-10">
              <p className="text-base leading-8 text-zinc-300 sm:text-lg md:text-[22px] md:leading-9">
                GRGN Creative; markalar için güçlü kimlikler, dikkat çekici
                görsel iletişim ve modern dijital deneyimler tasarlayan yaratıcı
                bir stüdyodur.
              </p>

              <p className="mt-6 text-sm leading-7 text-zinc-400 sm:text-base sm:leading-8 md:text-lg md:leading-8">
                Estetikle stratejiyi bir araya getirerek, markaların daha güçlü
                görünmesini, daha net algılanmasını ve dijitalde daha etkili bir
                iz bırakmasını hedefliyoruz.
              </p>
            </div>
          </Reveal>
        </div>
      </section>

      <section
        id="contact"
        className="snap-section relative flex min-h-[100dvh] items-center overflow-hidden px-5 py-20 sm:px-6 md:px-10 md:py-28"
      >
        <div className="absolute inset-0 bg-[linear-gradient(180deg,#02040b_0%,#030611_45%,#010208_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(55,90,210,0.10),transparent_24%),radial-gradient(circle_at_82%_30%,rgba(255,255,255,0.05),transparent_18%),radial-gradient(circle_at_72%_80%,rgba(45,80,200,0.08),transparent_28%)]" />
        <div className="absolute inset-0 opacity-[0.04] [background-image:radial-gradient(rgba(255,255,255,0.8)_0.45px,transparent_0.45px)] [background-size:4px_4px]" />

        <div className="absolute left-[-8%] bottom-[8%] h-[360px] w-[360px] rounded-full bg-blue-500/10 blur-3xl" />
        <div className="absolute right-[-10%] top-[10%] h-[420px] w-[420px] rounded-full bg-white/5 blur-3xl" />

        <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 md:grid-cols-[1fr_1.1fr] md:gap-20">
          <Reveal>
            <div>
              <p className="mb-4 text-sm font-medium uppercase tracking-[0.22em] text-white/45">
                İletişim
              </p>

              <h2 className="text-4xl font-semibold tracking-tight text-white sm:text-5xl md:text-6xl">
                Birlikte güçlü işler üretelim.
              </h2>

              <p className="mt-6 max-w-xl text-base leading-7 text-zinc-400 sm:text-lg sm:leading-8 md:text-[22px] md:leading-9">
                Yeni bir marka, güçlü bir kimlik ya da dijitalde daha etkili bir
                görünüm oluşturmak istiyorsanız, konuşalım.
              </p>

              <div className="mt-6 h-px w-24 bg-white/15" />
            </div>
          </Reveal>

          <Reveal delay={140}>
            <div className="rounded-[30px] border border-white/10 bg-white/[0.03] p-7 shadow-[0_0_40px_rgba(255,255,255,0.03)] backdrop-blur-sm sm:p-8 md:p-10">
              <div className="space-y-5">
                <ContactCard
                  href="mailto:berkaygirginjobs@gmail.com"
                  label="E-Posta"
                  value="berkaygirginjobs@gmail.com"
                />

                <ContactCard
                  href="https://wa.me/905301159374"
                  label="WhatsApp"
                  value="+90 530 115 93 74"
                  external
                />

                <ContactCard
                  href="https://instagram.com/grgncreative"
                  label="Instagram"
                  value="@grgncreative"
                  external
                />
              </div>

              <p className="mt-8 text-sm leading-7 text-zinc-500 md:text-base">
                Proje talepleri, iş birlikleri ve marka odaklı yaratıcı
                çalışmalar için bize ulaşabilirsiniz.
              </p>
            </div>
          </Reveal>
        </div>
      </section>

      <a
        href="https://wa.me/905301159374?text=Merhaba%2C%20web%20siteniz%20%C3%BCzerinden%20ula%C5%9F%C4%B1yorum."
        target="_blank"
        rel="noreferrer"
        className="fixed bottom-4 right-4 z-[999] group flex items-center gap-3 rounded-full border border-white/12 bg-black/45 px-3 py-2.5 text-white shadow-[0_10px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl transition duration-300 hover:scale-[1.03] hover:border-white/20 hover:bg-white/[0.06] sm:bottom-8 sm:right-8 sm:px-4 sm:py-3"
      >
        <div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.06] text-white shadow-inner sm:h-10 sm:w-10">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className="h-4 w-4"
          >
            <path d="M20.52 3.48A11.86 11.86 0 0 0 12.06 0C5.5 0 .16 5.34.16 11.9c0 2.1.55 4.16 1.6 5.98L0 24l6.3-1.65a11.84 11.84 0 0 0 5.76 1.47h.01c6.56 0 11.9-5.34 11.9-11.9 0-3.18-1.24-6.16-3.45-8.44ZM12.07 21.8h-.01a9.83 9.83 0 0 1-5.01-1.37l-.36-.21-3.74.98 1-3.64-.24-.38a9.8 9.8 0 0 1-1.5-5.27c0-5.43 4.42-9.85 9.86-9.85 2.63 0 5.1 1.02 6.95 2.88a9.78 9.78 0 0 1 2.88 6.96c0 5.44-4.42 9.86-9.84 9.86Zm5.4-7.37c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.95 1.17-.17.2-.35.22-.65.08-.3-.15-1.28-.47-2.43-1.5-.9-.8-1.5-1.8-1.68-2.1-.18-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.18.2-.3.3-.5.1-.2.05-.38-.02-.53-.08-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.5h-.57c-.2 0-.52.08-.8.38-.27.3-1.04 1.02-1.04 2.5 0 1.47 1.07 2.9 1.22 3.1.15.2 2.1 3.2 5.08 4.49.71.31 1.27.5 1.71.64.72.23 1.37.2 1.88.12.57-.08 1.77-.72 2.02-1.42.25-.7.25-1.31.18-1.43-.07-.12-.27-.2-.57-.35Z" />
          </svg>
        </div>

        <div className="flex flex-col leading-tight">
          <span className="text-[10px] uppercase tracking-[0.18em] text-white/45 sm:text-[11px]">
            Hızlı İletişim
          </span>
          <span className="text-[13px] font-medium text-white/90 sm:text-sm">
            WhatsApp
          </span>
        </div>
      </a>
    </main>
  );
}

function ServiceCard({
  img,
  title,
  desc,
  href,
}: {
  img: string;
  title: string;
  desc: string;
  href: string;
}) {
  return (
    <Link href={href} className="group block cursor-pointer">
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-zinc-900">
        <img
          src={img}
          alt={title}
          className="h-[220px] w-full object-cover transition duration-700 group-hover:scale-110"
        />

        <div className="absolute inset-0 bg-black/20 transition duration-500 group-hover:bg-black/40" />
      </div>

      <div className="mt-4">
        <h3 className="text-xl font-medium transition group-hover:text-white/90 sm:text-2xl">
          {title}
        </h3>
        <p className="mt-1 text-sm leading-7 text-zinc-400 sm:text-base">
          {desc}
        </p>
      </div>
    </Link>
  );
}

function ProjectCard({
  img,
  title,
  category,
  desc,
  href,
}: {
  img: string;
  title: string;
  category: string;
  desc: string;
  href: string;
}) {
  return (
    <Link href={href} className="group block h-full cursor-pointer">
      <div className="flex h-full flex-col overflow-hidden rounded-[24px] border border-white/10 bg-white/[0.03] backdrop-blur-sm transition duration-300 hover:border-white/20 hover:bg-white/[0.05]">
        <div className="relative overflow-hidden">
          <img
            src={img}
            alt={title}
            className="h-[220px] w-full object-cover transition duration-700 group-hover:scale-105 sm:h-[240px]"
          />
          <div className="absolute inset-0 bg-black/20 transition duration-500 group-hover:bg-black/35" />
        </div>

        <div className="flex flex-1 flex-col p-5 sm:p-6">
          <p className="text-xs uppercase tracking-[0.18em] text-white/40 sm:text-sm">
            {category}
          </p>

          <h3 className="mt-3 text-xl font-medium text-white sm:text-2xl">
            {title}
          </h3>

          <p className="mt-3 min-h-[84px] text-sm leading-7 text-zinc-400 sm:min-h-[96px] sm:text-base">
            {desc}
          </p>

          <div className="mt-auto flex items-center justify-between border-t border-white/10 pt-5">
            <span className="text-sm text-white/55">Projeyi incele</span>
            <span className="text-white/45 transition duration-300 group-hover:translate-x-1 group-hover:text-white">
              →
            </span>
          </div>
        </div>
      </div>
    </Link>
  );
}

function ContactCard({
  href,
  label,
  value,
  external = false,
}: {
  href: string;
  label: string;
  value: string;
  external?: boolean;
}) {
  return (
    <a
      href={href}
      target={external ? "_blank" : undefined}
      rel={external ? "noreferrer" : undefined}
      className="group relative flex items-center justify-between overflow-hidden rounded-2xl border border-white/10 bg-black/20 px-5 py-4 transition duration-300 hover:scale-[1.01] hover:border-white/20 hover:bg-white/[0.06] sm:px-6 sm:py-5"
    >
      <div className="pointer-events-none absolute inset-0 translate-x-[-120%] bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 transition duration-700 group-hover:translate-x-[120%] group-hover:opacity-100" />

      <div className="relative z-10 min-w-0">
        <p className="text-xs uppercase tracking-[0.18em] text-white/40 sm:text-sm">
          {label}
        </p>
        <p className="mt-2 break-all text-base font-medium text-white sm:text-lg md:text-xl">
          {value}
        </p>
      </div>

      <span className="relative z-10 ml-4 shrink-0 text-white/40 transition duration-300 group-hover:text-white">
        →
      </span>
    </a>
  );
}